...
public class InLocalSmallPhotoActivity extends Activity {
	...
	private void initComponents() {
		...
		gv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				try {
					Intent i = new Intent(InLocalSmallPhotoActivity.this,
							ImageBrowserActivity.class);
					i.putExtra("currentpic", arg2);
					i.putExtra("albumArray", albumArray);
					i.putExtra("photopath", PhotoAlbumActivity.AlbumsFloderPath
							.get(AlbumName).get(arg2));
					i.putExtra("AlbumName", AlbumName);
					startActivity(i);
				} catch (Exception e) {
					Toast.makeText(InLocalSmallPhotoActivity.this,
							"图像载入出错了~~~~", 0).show();
				}
			}
		});// ClickListener
	...
	Runnable rLocalAlbum = new Runnable() {
		...
		public void run() {
			...
			int albumSize = PhotoAlbumActivity.AlbumsFloderPath.get(AlbumName)
					.size();
			for (int i = 0; i < albumSize && !Thread.interrupted(); i++) {
				bm = decodeBitmap(PhotoAlbumActivity.AlbumsFloderPath.get(
						AlbumName).get(i)); // 取得图片
				sendMessage(bm, i);
			}
		}
